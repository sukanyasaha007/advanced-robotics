...
class DiscretizedSolver(object):
    ...
    def populate_transition_and_reward_funcs(self):
        ...
        # state env set
        # for pos, velocity in self.env.action_space:
        #     action = self.solver.get_action(self.get_state_index_from_coordinates(pos))
    ...
    def get_discrete_state_probabilities(self, continuous_state_vector):
        ...
        if self._mode == 'nn':
            ...
            for i, nn in enumerate([nn1, nn2, nn3, nn4, nn5, nn6, nn7, nn8]):
                ...
                if check_bins(nn):
                    nearest_neigh[i] = np.array((int(nn_id), 1/distance(continuous_state_vector, nn)))
            ...
            # get inverse distance for prob for possible nn
            nearest_neigh[:, 1] = np.divide(1, nearest_neigh[:, 1], out=np.zeros_like(nearest_neigh[:, 1]), where=nearest_neigh[:, 1]!=0)# take inverse of the distance
            # create prob distribution norm
            nearest_neigh[:, 1] /= nearest_neigh[:, 1].sum()
            nearest_neigh[:, 0] = nearest_neigh[:, 0].astype(np.int32)
            nearest_neigh = np.unique(list(map(tuple, nearest_neigh)))
            # nearest_neigh= nearest_neigh[np.where(nearest_neigh[:, 0] < 441)]
            # print(nearest_neigh)
            return nearest_neigh
            


    def compute_policy(self, max_iterations= 20):
        '''
        ...
        for i in range(max_iterations):
            ...
            for s in range(n_states):
                ...
                '''
            # if np.sqrt((np.square(v_i - v_i_plus_1)).sum()) < .00000001: break
            v_i= v_i_plus_1
    ...
    def solve(self, visualize=False, max_steps=float('inf')):
        ...
        while finished is False and num_steps < max_steps:
            ...
            if self._lookahead_steps > 0:
                ...
                action_sequences = list(itertools.product([0,1,2], repeat= 3)) # TODO: Replace this line!
